...
import { Order, Market, OpenOrders, Orderbook, Side } from '@bonfida/dex-v4';
...
export function useOrderbookAccounts() {
  ...
  useEffect(() => {
    ...
    Orderbook.load(connection, market.address).then(setOrderbook);
  }, [market]);
  ...
  return fills
    .filter((fill) =>
      openOrdersAccounts.some((openOrdersAccount) =>
        fill.openOrders.equals(openOrdersAccount.address),
      ),
    )
  ...
  useEffect(() => {
    if (connected && wallet) {
      const getAllOpenOrders = async () => {
        setLoaded(false);
        const _openOrders: { orders: any[]; marketAddress: string }[] = [];
...
export function getMarketOrderPrice(
  ...
  const asks = orderbook.getL2(1000, true);
  for ({ price, size: sizeAtLevel } of asks) {
