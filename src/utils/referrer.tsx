...
export const useFeesAccountsFromRefCode = (refCode: string | undefined) => {
  ...
  const [atlas, setAtlas] = useState<PublicKey | undefined>(undefined);

  useEffect(() => {
    const fn = async () => {
      let usdcAddress: undefined | PublicKey = undefined;
      let usdtAddress: undefined | PublicKey = undefined;
      let atlasAddress: undefined | PublicKey = undefined;
      if (!refCode) {
        setUsdc(
        ...
        setAtlas(
            process.env.REACT_APP_ATLAS_REFERRAL_FEES_ADDRESS
                ? new PublicKey(process.env.REACT_APP_ATLAS_REFERRAL_FEES_ADDRESS)
                : undefined,
        );
        return;
      }
      try {
        ...
        atlasAddress = await findAssociatedTokenAddress(
            nameRegistryState.owner,
            ATLAS_MINT
        )
        ...
        const atlasAccountInfo = await connection.getAccountInfo(atlasAddress);
        if (!!usdcAccountInfo?.data) setUsdc(usdcAddress);
        if (!!usdtAccountInfo?.data) setUsdt(usdtAddress);
        if (!!atlasAccountInfo?.data) setAtlas(atlasAddress);
      } catch (err) {
  ...
  return { usdcRef: usdc, usdtRef: usdt, atlasRef: atlas };
};
...
export const ReferrerProvider = ({ children }) => {
  ...
  const { usdcRef, usdtRef, atlasRef } = useFeesAccountsFromRefCode(
  ...
  return (
    <ReferrerContext.Provider
      value={{ usdcRef, usdtRef, atlasRef, setRefCode, refCode, allowRefLink }}
    >
      {children}
    </ReferrerContext.Provider>
  );
